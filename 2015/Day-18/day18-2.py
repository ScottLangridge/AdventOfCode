...
class Grid:
    def __init__(self, raw_input):
        split_raw_input = raw_input.splitlines()

        self._grid = {}
        for y, row in enumerate(split_raw_input):
            for x, light in enumerate(row):
                self._grid[(x, y)] = light == '#'

        max_x = len(split_raw_input[0]) - 1
        max_y = len(split_raw_input) - 1

        self.always_on = [(0, 0), (max_x, 0), (0, max_y), (max_x, max_y)]
        self.set_always_on()
